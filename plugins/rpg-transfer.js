...
handler.before = async (m, { conn }) => {
    ...
    if (/no?/g.test(m.text.toLowerCase())) {
        ...
        return conn.sendButton(m.chat, `*–––––『 CANCELLED 』–––––*`, `ᴛʀᴀɴsғᴇʀ ᴄᴀɴᴄᴇʟʟᴇᴅ ᴏғ *${count}* ${rpg.emoticon(type)}${type}${special(type)} ᴛᴏ *@${(to || '').replace(/@s\.whatsapp\.net/g, '')}*`, imgr + 'cancelled',
[
[`ɪɴᴠᴇɴᴛᴏʀʏ`, `/inventory`]
    ...
    if (/y(es)?/g.test(m.text.toLowerCase())) {
        ...
        user[type] -= count * 1
        _user[type] += count * 1
        if (previous > user[type] * 1 && _previous < _user[type] * 1) conn.sendButton(m.chat, `*––––『 TRANSFERRED 』––––*`, `sᴜᴄᴄᴇssғᴜʟʟʏ ᴛʀᴀɴsғᴇʀ *${count}* ${rpg.emoticon(type)}${type}${special(type)} ᴛᴏ *@${(to || '').replace(/@s\.whatsapp\.net/g, '')}*`, imgr + 'transferred',
[
[`ɪɴᴠᴇɴᴛᴏʀʏ`, `/inventory`]
], m, { mentions: [to], asLocation: true })
        else {
